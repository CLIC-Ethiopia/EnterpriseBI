...
const App: React.FC = () => {
  ...
  const handleSwitchDepartment = (dept: DepartmentData) => {
    if (activeDepartment?.id === dept.id) return; 
    setSelectedDeptForLogin(dept);
    setShowLoginModal(true);
  };
  ...
  const handlePrint = () => {
    // If we are in the dashboard and have a department, show the custom report
  ...
  return (
    <div className={`min-h-screen ${viewState === 'LANDING' ? '' : 'bg-gray-50 dark:bg-gray-900'} font-sans text-gray-900 dark:text-gray-100 selection:bg-indigo-100 selection:text-indigo-900 transition-colors duration-300 main-app-container`}>

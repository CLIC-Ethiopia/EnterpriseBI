...
import { 
  ...
  FileBarChart, FileText, Hash, Calendar, ChevronDown, Building2, Mail, ShieldCheck
} from 'lucide-react';
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 backdrop-blur-sm bg-black/70 print:static print:bg-white print:p-0 print:block print:h-auto print:absolute print:inset-0 print:z-[9999] report-print-container">
           ...
           <div className="bg-gray-100 dark:bg-gray-900 rounded-2xl w-full max-w-4xl h-[90vh] shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200 print:shadow-none print:w-full print:max-w-none print:h-auto print:overflow-visible print:rounded-none">
              ...
              <div className="flex-1 overflow-y-auto p-8 bg-gray-200 dark:bg-gray-900 flex justify-center print:overflow-visible print:p-0 print:block print:bg-white print:h-auto">
                 <div id="report-content" className="bg-white text-black w-full max-w-[210mm] min-h-[297mm] p-[15mm] shadow-xl print:shadow-none print:w-full print:max-w-none print:p-0 print:mx-0 print:my-0 flex flex-col relative print-exact">
                    
                    {/* Report Header */}
                    <div className="flex justify-between items-start border-b-4 border-indigo-900 pb-6 mb-8 print:break-inside-avoid break-inside-avoid">
                       <div className="flex items-center gap-4">
                          <div className="w-14 h-14 bg-indigo-900 rounded-lg flex items-center justify-center text-white print-exact">
                             <Globe className="w-8 h-8" />
                          </div>
                          <div>
                             <h1 className="text-2xl font-bold uppercase tracking-tight text-gray-900">GlobalTrade<span className="text-indigo-700">BI</span></h1>
                             <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mt-1">Enterprise Intelligence</p>
                          </div>
                       </div>
                       <div className="text-right">
                          <h2 className="text-xl font-bold text-indigo-900 uppercase">Performance Report</h2>
                          <p className="text-sm font-semibold text-gray-600">{department.name}</p>
                          <p className="text-xs text-gray-500 mt-1">
                             Period: {reportConfig.period === 'Custom' ? `${reportConfig.startDate} - ${reportConfig.endDate}` : reportConfig.period}
                          </p>
                       </div>
                    </div>

                    {/* Meta Info Bar */}
                    <div className="flex justify-between text-xs text-gray-600 bg-gray-100 p-3 rounded mb-8 border border-gray-200 print-exact print:break-inside-avoid break-inside-avoid">
                       <div className="flex gap-4">
                          <span><strong>Generated By:</strong> {currentUser.name} ({currentUser.role})</span>
                          <span><strong>Date:</strong> {new Date().toLocaleDateString()}</span>
                       </div>
                       <div>
                          <span><strong>Report ID:</strong> {Math.floor(Math.random() * 1000000)}</span>
                       </div>
                    </div>

                    {/* Executive Summary */}
                    <section className="mb-10 print:break-inside-avoid break-inside-avoid">
                       <h3 className="text-sm font-bold text-indigo-900 border-b border-gray-300 pb-2 mb-4 uppercase tracking-wider">Executive Summary</h3>
                       <p className="text-sm text-gray-800 leading-relaxed text-justify">
                          This report outlines the operational and financial performance of the <strong>{department.name}</strong> department for the specified period. 
                          Analysis of key performance indicators reveals a <strong>{department.kpis[0]?.trend === 'up' ? 'positive upward trend' : 'stabilization'}</strong> in primary metrics. 
                          The total assessed value currently stands at <strong>{department.kpis[0]?.value}</strong>, reflecting a {department.kpis[0]?.change} deviation from the previous benchmark.
                          Strategic focus remains on optimizing efficiency and mitigating risks associated with {department.kpis[2]?.label || 'operational variances'}.
                       </p>
                    </section>

                    {/* KPI Grid */}
                    <section className="mb-10 print:break-inside-avoid break-inside-avoid">
                       <h3 className="text-sm font-bold text-indigo-900 border-b border-gray-300 pb-2 mb-4 uppercase tracking-wider">Key Metrics</h3>
                       <div className="grid grid-cols-4 gap-4">
                          {department.kpis.map((kpi, i) => (
                             <div key={i} className="border border-gray-200 rounded p-3 bg-gray-50 print-exact">
                                <p className="text-[10px] uppercase font-bold text-gray-500 mb-1">{kpi.label}</p>
                                <p className="text-xl font-bold text-gray-900">{kpi.value}</p>
                                <div className="flex items-center gap-1 mt-1">
                                   <span className={`text-xs font-bold ${kpi.trend === 'up' ? 'text-green-700' : 'text-red-700'}`}>
                                      {kpi.change}
                                   </span>
                    ...
                    {/* Visual Analysis (Static Chart) */}
                    <section className="mb-10 print:break-inside-avoid break-inside-avoid">
                        <h3 className="text-sm font-bold text-indigo-900 border-b border-gray-300 pb-2 mb-4 uppercase tracking-wider">Trend Analysis</h3>
                        <div className="h-64 w-full border border-gray-200 rounded p-4">
                           {/* Using Recharts without animation for clean print render */}
                           <ResponsiveContainer width="100%" height="100%">
                              <AreaChart data={department.mainChartData}>
                        ...
                        <p className="text-xs text-gray-500 mt-2 italic text-center">Fig 1.1 - Performance trend over current reporting period.</p>
                    </section>

                    {/* Detailed Data Table */}
                    <section className="mb-10 print:break-inside-avoid break-inside-avoid">
                       <h3 className="text-sm font-bold text-indigo-900 border-b border-gray-300 pb-2 mb-4 uppercase tracking-wider">Operational Data</h3>
                       <table className="w-full text-xs text-left border-collapse border border-gray-200">
                          <thead>
                             <tr className="bg-gray-100 text-gray-700 print-exact">
                                <th className="p-2 border border-gray-200 font-bold">Item Identifier</th>
                                <th className="p-2 border border-gray-200 font-bold">Category</th>
                                <th className="p-2 border border-gray-200 font-bold text-center">Status</th>
                                <th className="p-2 border border-gray-200 font-bold text-right">Value</th>
                                <th className="p-2 border border-gray-200 font-bold text-right">Completion</th>
                             </tr>
                          </thead>
                          <tbody>
                             {department.summaryTableData.slice(0, 10).map((row, i) => (
                                <tr key={i} className="border-b border-gray-100">
                                   ...
                                   <td className="p-2 border border-gray-200 text-center">
                                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold border print-exact ${
                                   ...
                                   <td className="p-2 border border-gray-200 text-right font-mono">{row.value}</td>
                                   <td className="p-2 border border-gray-200 text-right">{row.completion}%</td>
                                </tr>
                             ))}
                          </tbody>
                       </table>
                    ...
                    <section className="mb-12 print:break-inside-avoid break-inside-avoid">
                        <h3 className="text-sm font-bold text-indigo-900 border-b border-gray-300 pb-2 mb-4 uppercase tracking-wider">Strategic Recommendations</h3>
                        <div className="bg-gray-50 border border-gray-200 rounded p-4 print-exact">
                           <ul className="list-disc list-inside text-sm text-gray-700 space-y-2">
                              {department.suggestedPrompts?.slice(0, 3).map((prompt, i) => (
                                 <li key={i}>{prompt.replace(/\.$/, '')}. Action recommended within Q4 timeframe.</li>
                              )) || (
                                 <li>Review operational overheads to align with fiscal targets.</li>
                              )}
                              <li>Maintain current growth trajectory by leveraging identified efficiencies in the supply chain.</li>
                           </ul>
                        </div>
                    </section>

                    {/* Footer */}
                    <footer className="mt-auto pt-8 border-t-2 border-gray-800 flex justify-between items-end print:break-inside-avoid break-inside-avoid">
                       <div>
                          <div className="flex items-center gap-2 mb-4">
                             <ShieldCheck className="w-4 h-4 text-gray-400" />
                             <span className="text-[10px] font-bold uppercase text-gray-500">Authorized Signature</span>
                          </div>
                          <div className="w-48 border-b border-black mb-1"></div>
                          <p className="text-[10px] text-gray-400">Director of Operations</p>
                       </div>
                       <div className="text-right">
                          <p className="text-[10px] font-bold uppercase text-gray-500 mb-1">Confidential Document</p>
                          <p className="text-[10px] text-gray-400">GlobalTrade Logistics Inc. • Generated on {new Date().toLocaleDateString()}</p>
